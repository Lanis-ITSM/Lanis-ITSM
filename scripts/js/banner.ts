...
(async () => {
	const generateBanners: IGenBanner[] = [];
	const posts = fs
		.readdirSync(blogsContent)
		...
		serve.stdout?.on('data', async (data) => {
			if (data.toString().includes('http://localhost:3000')) {
				...
				for (const { slug, bannerPath, isBlog } of generateBanners) {
					console.log(
						`[banner] Generating banner for ${isBlog ? 'blog' : 'snippets'} => ${slug}`,
					);
					...
					await page.goto(
						`http://localhost:3000/${isBlog ? 'blog' : 'snippets'}/${slug}`,
					);
					if (first) {
						// to hide BMC message
						await page.goto(
					...
					await page.$eval('#blog-conent', (el) => {
						...
						twitterDiv.className =
							'flex flex-col mt-10 justify-start items-start md:items-center w-full';
